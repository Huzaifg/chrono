...
def predict(device: str):
  ...
  simulator = _get_simulator(metadata, FLAGS.noise_std, FLAGS.noise_std, device, FLAGS.con_radius)
...
def train(rank, flags, world_size, device):
  ...
  if device == torch.device("cuda"):
    serial_simulator = _get_simulator(metadata, flags["noise_std"], flags["noise_std"], rank, flags["con_radius"])
...
def _get_simulator(
        ...
        vel_noise_std: float,
        device: torch.device,
        con_radius: float) -> learned_simulator.LearnedSimulator:
  ...
  simulator = learned_simulator.LearnedSimulator(
      ...
      nmlp_layers=2,
      mlp_hidden_dim=128,
      connectivity_radius=con_radius,
