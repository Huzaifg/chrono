...
void PrintParticleToFile(const thrust::device_vector<Real4>& posRadD,
    ...
    if(!withAccComp){
        PrintParticleToFile(posRadD, velMasD, rhoPresMuD, sr_tau_I_mu_i, derivVelRhoD, referenceArray, referenceArrayFEA, dir, paramsH);
        return;
    }
    thrust::host_vector<Real4> posRadH = posRadD;
    thrust::host_vector<Real3> velMasH = velMasD;
    ...
    if (haveHelper || haveGhost) {
        ...
        {
            if (out_length == 0) {
                ssotherParticles << "x,y,z,|U|,acc_x,acc_y,acc_z,acc\n";
            } else if (out_length == 1) {
                ssotherParticles << "x,y,z,v_x,v_y,v_z,|U|,acc_x,acc_y,acc_z,acc,rho,pressure\n";
            } else if (out_length == 2) {
                ssotherParticles << "x,y,z,h,v_x,v_y,v_z,|U|,acc_x,acc_y,acc_z,acc,rho(rpx),p(rpy),mu(rpz),sr,tau,I,mu_i,type(rpw)\n";
            }
        }
        for (size_t i = referenceArray[0].x; i < referenceArray[haveHelper + haveGhost].y; i++) {
            Real4 rP = rhoPresMuH[i];
            ...
            Real accMag = length(acc);
            if (out_length == 0) {
                ssotherParticles << pos.x << ", " << pos.y << ", " << pos.z << ", " << velMag + eps << ", " << acc.x << ", " << acc.y << ", " << acc.z << ", " << accMag << std::endl;
            } else if (out_length == 1) {
                ssotherParticles << pos.x << ", " << pos.y << ", " << pos.z << ", " << vel.x + eps << ", "
                                 << vel.y + eps << ", " << vel.z + eps << ", " << velMag + eps << ", " << acc.x << ", " << acc.y << ", " << acc.z  << ", " << accMag << ", " << rP.x << ", "
                                 << rP.y + eps << std::endl;
            } else if (out_length == 2) {
                ssotherParticles << pos.x << ", " << pos.y << ", " << pos.z << ", " << pos.w << ", " << vel.x + eps
                                 << ", " << vel.y + eps << ", " << vel.z + eps << ", " << velMag + eps << ", " << acc.x << ", " << acc.y << ", " << acc.z << ", " << accMag << ", " << rP.x
    ...
    {
        if (out_length == 0) {
            ssFluidParticles << "x,y,z,|U|,acc_x,acc_y,acc_z,acc\n";
        } else if (out_length == 1) {
            ssFluidParticles << "x,y,z,v_x,v_y,v_z,|U|,acc_x,acc_y,acc_z,acc,rho,pressure\n";
        } else if (out_length == 2) {
            ssFluidParticles << "x,y,z,h,v_x,v_y,v_z,|U|,acc_x,acc_y,acc_z,acc,rho(rpx),p(rpy),mu(rpz),sr,tau,I,mu_i,type(rpw)\n";
        }
    }
    ...
    for (size_t i = referenceArray[haveHelper + haveGhost].x; i < referenceArray[haveHelper + haveGhost].y; i++) {
        ...
        Real accMag = length(acc);
        if (out_length == 0) {
            ssFluidParticles << pos.x << ", " << pos.y << ", " << pos.z << ", " << velMag + eps << ", " << acc.x << ", " << acc.y << ", " << acc.z << ", " << accMag << std::endl;
        } else if (out_length == 1) {
            ssFluidParticles << pos.x << ", " << pos.y << ", " << pos.z << ", " << vel.x + eps << ", "
                             << vel.y + eps << ", " << vel.z + eps << ", " << velMag + eps << ", " << acc.x << ", " << acc.y << ", " << acc.z << ", " << accMag << ", " << rP.x << ", "
                             << rP.y + eps << std::endl;
    ...
    if (frame_num == 0) {
        ...
        {
            if (out_length == 0) {
                ssFluidBoundaryParticles << "x,y,z,|U|,acc_x,acc_y,acc_z,acc\n";
            } else if (out_length == 1) {
                ssFluidBoundaryParticles << "x,y,z,v_x,v_y,v_z,|U|,acc_x,acc_y,acc_z,acc,rho,pressure\n";
            } else if (out_length == 2) {
                ssFluidBoundaryParticles << "x,y,z,h,v_x,v_y,v_z,|U|,acc_x,acc_y,acc_z,acc,rho(rpx),p(rpy),mu(rpz),sr,tau,I,mu_i,type(rpw)\n";
            }
        }
        ...
        for (size_t i = referenceArray[haveHelper + haveGhost + 1].x; i < referenceArray[haveHelper + haveGhost + 1].y; i++) {
            ...
            Real accMag = length(acc);
            if (out_length == 0) {
                ssFluidBoundaryParticles << pos.x << ", " << pos.y << ", " << pos.z << ", " << velMag + eps << ", " << acc.x << ", " << acc.y << ", " << acc.z << ", " << accMag << std::endl;
            } else if (out_length == 1) {
                ssFluidBoundaryParticles << pos.x << ", " << pos.y << ", " << pos.z << ", " << vel.x + eps << ", "
                                         << vel.y + eps << ", " << vel.z + eps << ", " << velMag + eps << ", " << acc.x << ", " << acc.y << ", " << acc.z << ", " << accMag << ", " << rP.x << ", "
                                         << rP.y + eps << std::endl;
                ...
                ssFluidBoundaryParticles << pos.x << ", " << pos.y << ", " << pos.z << ", " << pos.w << ", "
                                         << vel.x + eps << ", " << vel.y + eps << ", " << vel.z + eps << ", "
                                         << velMag + eps << ", " << acc.x << ", " << acc.y << ", " << acc.z << ", " << accMag << ", " << rP.x << ", " << rP.y + eps << ", " << rP.z << ", "
                                         << stIm.x << ", " << stIm.y + eps << ", " << stIm.z << ", " << stIm.w << ", "
                                         << rP.w << std::endl;
            }
        }
    ...
    if (refSize > haveHelper + haveGhost + 2 && referenceArray[2].z == 1) {
        ...
        {
            if (out_length == 0) {
                ssBCE << "x,y,z,|U|,acc_x,acc_y,acc_z,acc\n";
            } else if (out_length == 1) {
                ssBCE << "x,y,z,v_x,v_y,v_z,|U|,acc_x,acc_y,acc_z,acc,rho,pressure\n";
            } else if (out_length == 2) {
                ssBCE << "x,y,z,h,v_x,v_y,v_z,|U|,acc_x,acc_y,acc_z,acc,rho(rpx),p(rpy),mu(rpz),sr,tau,I,mu_i,type(rpw)\n";
            }
        }
        ...
        for (size_t i = referenceArray[haveHelper + haveGhost + 2].x; i < referenceArray[refSize - 1].y; i++) {
            ...
            if (rP.w == 1.0) {
                if (out_length == 0) {
                    ssBCE << pos.x << ", " << pos.y << ", " << pos.z << ", " << velMag + eps << ", " << acc.x << ", " << acc.y << ", " << acc.z << ", " << accMag << std::endl;
                } else if (out_length == 1) {
                    ssBCE << pos.x << ", " << pos.y << ", " << pos.z << ", " << vel.x + eps << ", "
                          << vel.y + eps << ", " << vel.z + eps << ", " << velMag + eps << ", " << acc.x << ", " << acc.y << ", " << acc.z << ", " << accMag << ", " << rP.x << ", "
                          << rP.y + eps << std::endl;
    ...
    if (refSize_Flex > 0) {
        ...
        {
            if (out_length == 0) {
                ssBCE_Flex << "x,y,z,|U|,acc_x,acc_y,acc_z,acc\n";
            } else if (out_length == 1) {
                ssBCE_Flex << "x,y,z,v_x,v_y,v_z,|U|,acc_x,acc_y,acc_z,acc,rho,pressure\n";
            } else if (out_length == 2) {
                ssBCE_Flex << "x,y,z,h,v_x,v_y,v_z,|U|,acc_x,acc_y,acc_z,acc,rho(rpx),p(rpy),mu(rpz),sr,tau,I,mu_i,type(rpw)\n";
            }
        }
        for (size_t i = referenceArrayFEA[0].x; i < referenceArrayFEA[refSize_Flex - 1].y; i++) {
            Real4 pos = posRadH[i];
            ...
            Real accMag = length(acc);
            if (out_length == 0) {
                ssBCE_Flex << pos.x << ", " << pos.y << ", " << pos.z << ", " << velMag + eps << ", " << acc.x << ", " << acc.y << ", " << acc.z << ", " << accMag << std::endl;
            } else if (out_length == 1) {
                ssBCE_Flex << pos.x << ", " << pos.y << ", " << pos.z << ", " << vel.x + eps << ", "
                           << vel.y + eps << ", " << vel.z + eps << ", " << velMag + eps << ", " << acc.x << ", " << acc.y << ", " << acc.z << ", " << accMag << ", " << rP.x << ", "
                           << rP.y + eps << std::endl;
