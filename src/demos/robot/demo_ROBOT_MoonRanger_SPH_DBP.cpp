...
void CreateSolidPhase(ChSystemSMC& sysMBS,
    ...
    auto driver = chrono_types::make_shared<MoonrangerSpeedDriver>(0.0, wheel_AngVel);
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (i == 1 || i == 3) {
            sysFSI.AddWheelBCE_Grouser(Wheels[i], ChFrame<>(VNULL, QuatFromAngleZ(CH_PI)), inner_radius,
                                       wheel_wide - iniSpacing, grouser_height, grouser_wide, grouser_num, iniSpacing,
                                       false,
                                       true);  // last true is for hollow wheel
        } else {
            sysFSI.AddWheelBCE_Grouser(Wheels[i], ChFrame<>(VNULL, QUNIT), inner_radius, wheel_wide - iniSpacing,
                                       grouser_height, grouser_wide, grouser_num, iniSpacing, false,
                                       true);  // last true is for hollow wheel
        }
